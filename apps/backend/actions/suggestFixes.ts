...
export const suggestFixes: SuggestFixes = async (opml) => {
  ...
  for (const outline of opml?.body?.outlines || []) {
    if (outline.xmlUrl) {
      ...
      const preparedUrl = prepareUrl(outline.xmlUrl, { validate: true })

      if (!preparedUrl) {
        console.warn('[SECURITY] Skipping unsafe URL from OPML:', {
          originalUrl: outline.xmlUrl,
          preparedUrl,
        })
        continue
      }

      opmlFeedUrls.push(preparedUrl)

      if (outline.htmlUrl) {
        opmlSiteUrls.set(preparedUrl, outline.htmlUrl)
      }
    }
  }
